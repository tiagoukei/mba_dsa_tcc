...
plt.xlabel('Número de Palavras')
plt.ylabel('Quantidade de Notas')
plt.xlim(0, max_palavras_equip_nota + 1)
plt.ylim(0, 4500)
plt.xticks(np.arange(0, max_palavras_equip_nota + 1, step=2))
aplicar_normas(ax)
plt.tight_layout()
...
comparacao_modelos_classe_refinados = pd.DataFrame(columns=['Modelo', 'Hiperparâmetros', 'Acurácia', 'Acurácia no treino', 'Acurácia Balanceada', 'Precisão', 'Recall', 'F1 Score', 'Tempo'])
for nome_modelo, modelo in modelos_refinados:
    ...
    accuracy_train = modelo.score(Xc_n_eqnt_bw_train, yc_n_train)
    ...
    print("Hiperparâmetros:", hiperparam)
    print(f"Acurácia no treino ({nome_modelo}):", accuracy_train)
    print(f"Acurácia no teste ({nome_modelo}):", accuracy)
    ...
    comparacao_modelos_classe_refinados.loc[len(comparacao_modelos_classe_refinados)] = [nome_modelo, 'GridSearch', accuracy, accuracy_train, balanced_accuracy, precision, recall, f1, end_time - start_time]
...
comparacao_modelos_falha_refinados = pd.DataFrame(columns=['Modelo', 'Hiperparâmetros', 'Acurácia', 'Acurácia no treino', 'Acurácia Balanceada', 'Precisão', 'Recall', 'F1 Score', 'Tempo'])
for nome_modelo, modelo in modelos_refinados:
    start_time = time.time()
    ...
    accuracy_train = modelo.score(Xf_n_nt_bw_train, yf_n_train)
    ...
    comparacao_modelos_falha_refinados.loc[len(comparacao_modelos_falha_refinados)] = [nome_modelo, 'GridSearch', accuracy, accuracy_train, balanced_accuracy, precision, recall, f1, end_time - start_time]
...
print("\n=== Avaliação do modelo roBERTa - Classes de Equipamento ===")
train_c_metrics = trainer_c.evaluate(train_c_ds)
print("\n=== Métricas finais (treino) ===")
for k, v in train_c_metrics.items():
...
print("\n=== Avaliação do modelo roBERTa - Modos de Falha ===")
train_f_metrics = trainer_f.evaluate(train_f_ds)
print("\n=== Métricas finais (treino) ===")
for k, v in train_f_metrics.items():
    if isinstance(v, float):
        print(f"{k}: {v:.4f}")
    else:
        print(f"{k}: {v}")
...
modelos_classe_acuracia = comparacao_modelos_classe_refinados.melt(
    id_vars="Modelo",
    value_vars=["Acurácia no treino", "Acurácia"],
    var_name="Amostra",
    value_name="Valor"
)
modelos_classe_acuracia['Amostra'] = modelos_classe_acuracia['Amostra'].map({
    'Acurácia no treino': 'Treino',
    'Acurácia': 'Teste'
})
roberta_classe_acuracia = pd.DataFrame([
    {'Modelo': 'roBERTa', 'Amostra': 'Teste', 'Valor': eval_c_metrics['eval_accuracy']},
    {'Modelo': 'roBERTa', 'Amostra': 'Treino', 'Valor': train_c_metrics['eval_accuracy']}
])
modelos_classe_acuracia = pd.concat([modelos_classe_acuracia, roberta_classe_acuracia], ignore_index=True)

# gráfíco
fig, ax = plt.subplots(figsize=(10, 6))
sns.barplot(
    data=modelos_classe_acuracia,
    x="Modelo", y="Valor", hue="Amostra",
    ax=ax
)
...
plt.ylabel('Acurácia')
plt.ylim(0, 1)
plt.legend(title='Amostra', loc='lower right')
plt.setp(plt.gca().get_xticklabels(), rotation=30, ha='right', rotation_mode='anchor')
aplicar_normas(ax)
plt.tight_layout()
...
modelos_classe_tempo = {
    ...
    'Gradient Boosting': comparacao_modelos_classe_refinados.loc[comparacao_modelos_classe_refinados['Modelo'] == 'Gradient Boosting', 'Tempo'].values[0],
...
modelos_falha_acuracia = comparacao_modelos_falha_refinados.melt(
    id_vars="Modelo",
    value_vars=["Acurácia no treino", "Acurácia"],
    var_name="Amostra",
    value_name="Valor"
)
modelos_falha_acuracia['Amostra'] = modelos_falha_acuracia['Amostra'].map({
    'Acurácia no treino': 'Treino',
    'Acurácia': 'Teste'
})
roberta_falha_acuracia = pd.DataFrame([
    {'Modelo': 'roBERTa', 'Amostra': 'Teste', 'Valor': eval_f_metrics['eval_accuracy']},
    {'Modelo': 'roBERTa', 'Amostra': 'Treino', 'Valor': train_f_metrics['eval_accuracy']}
])
modelos_falha_acuracia = pd.concat([modelos_falha_acuracia, roberta_falha_acuracia], ignore_index=True)

# gráfico
fig, ax = plt.subplots(figsize=(10, 6))
sns.barplot(
    data=modelos_falha_acuracia,
    x="Modelo", y="Valor", hue="Amostra",
    ax=ax
)
for container in ax.containers:
...
modelos_falha_tempo = {
    ...
    'Gradient Boosting': comparacao_modelos_falha_refinados.loc[comparacao_modelos_falha_refinados['Modelo'] == 'Gradient Boosting', 'Tempo'].values[0],
